...
def create_npc(x, y, z, scale, player):
    """Создаёт NPC из предварительно масштабированных координат. | Create NPC from pre-scaled coordinates."""
    return AnimatedNPC(
        start_pos=(x, y, z),
        player=player,
...
def load_map(filename=MAP_FILENAME, player=None, load_npcs=True, verbose=True):
    ...
    Возвращает | Returns:
        tuple: (world_entities, npcs, player_start)
    """
    world_entities = []
    npcs = []
    ...
    if load_npcs:
        for npc_data in data.get("npcs", []):
            try:
                ...
                x, z = _clamp_coords(x, z, map_half)
                if player:
                    npc = create_npc(x, y, z, npc_data.get("scale", DROID_SCALE), player)
                    npcs.append(npc)
                    world_entities.append(npc)
            except Exception as e:
                if verbose:
                    print(f"  ❌ Ошибка загрузки NPC: {e} | Error loading NPC: {e}")
